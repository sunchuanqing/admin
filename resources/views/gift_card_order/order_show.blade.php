...
@section('content')
                    ...
                    <div class="layui-col-md6">
                        <div class="layui-card">
                            ...
                            <div class="layui-card-body">
                                ...
                                订单类别：@if ($data->order_type === 1)奢饰品护理@elseif ($data->order_type === 2)名车护理@elseif ($data->order_type === 3)花艺@elseif ($data->order_type === 4)优惠券@elseif ($data->order_type === 5)优惠服务@elseif ($data->order_type === 6)好货@elseif ($data->order_type === 7)车护年卡@elseif ($data->order_type === 8)礼品卡@endif<br>
                                    ...
                                    <div class="layui-form-item">
                                        <div class="layui-input-block" style="margin-left: 0px;">
                                            ...
                                            @elseif ($data->pay_status === 3)
                                                ...
                                                @elseif (($data->shipping_status === 5) || ($data->shipping_status === 8))
                                                    <a href="javascript:goods_finish();" class="layui-btn">确认收货</a>
                                                @elseif ($data->shipping_status === 6)
                                                    <a href="javascript:void(0);" class="layui-btn layui-btn-disabled">用户已确认收货</a>
                                                @elseif ($data->shipping_status === 9)
                                                    <a href="javascript:void(0);" class="layui-btn layui-btn-disabled">已发放至用户账户</a>
                                                @endif
                                            @endif
                                        </div>
                                    </div>
    ...
    <script>
        ...
        function goods_finish(){
            layer.confirm('请确认用户已经收到物品', {
                btn: ['确认','取消'] //按钮
            }, function(){
                var index = layer.load(2, {shade: [0.1, '#000000']});
                $.ajax({
                    type: 'post',
                    url: '/goods_finish',
                    data: {'_token': '{{csrf_token()}}', 'order_id': $("#id").val(), 'to_buyer': $("#to_buyer").val()},
                    dataType: 'json',
                    success: function(data){
                        layer.close(index);
                        if(data.code === 200){
