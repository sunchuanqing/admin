        ...
        <div class="add">
            <form class="layui-form" action="">
                    ...
                    <div class="layui-input-block">
                        <select name="type_id" lay-verify="required" lay-filter="banner_type">
                            ...
                            @foreach($banner_type as $k => $v)
                                <option value="{{$v->id}}.{{$v->banner_width}}.{{$v->banner_height}}" {{$v->id == $data['type_id'] ? 'selected' : ''}}>{{$v->name}}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
                <div class="layui-form-item">
                    ...
                    <div class="layui-input-block">
                        <input type="text" name="name" value="{{$data['name']}}" required  lay-verify="required" placeholder="轮播图名称" autocomplete="off" class="layui-input">
                    </div>
                </div>
                <div class="layui-form-item">
                    ...
                    <div class="layui-input-block">
                        ...
                        <span id="beizhu" style="color: #666;">图片尺寸 宽度为 :{{$type['banner_width']}}px; 高度为 :{{$type['banner_height']}}px;</span>
                        <div class="layui-upload-list">
                            <img id="imgurls" style="width: calc({{$type['banner_width']}}px / 5); height: calc({{$type['banner_height']}}px / 5);" src="{{$data['img_url']}}" alt="">
                            <input type="hidden" name="img_url" value="{{$data['img_url']}}" lay-verify="required" id="img_url">
                        </div>
                    </div>
                </div>
                <div class="layui-form-item">
                    ...
                    <div class="layui-input-block">
                        <input type="text" name="link" value="{{$data['link']}}" required  lay-verify="required" placeholder="跳转链接" autocomplete="off" class="layui-input">
                    </div>
                </div>
                <div class="layui-form-item">
                ...
                <div class="layui-form-item">
                    ...
                    <div class="layui-input-block">
                        <input type="radio" name="status" value="1" title="启用" {{1 == $data['status'] ? 'checked' : ''}}>
                        <input type="radio" name="status" value="2" title="禁用" {{2 == $data['status'] ? 'checked' : ''}}>
                    </div>
                </div>
                <div class="layui-form-item">
                    <div class="layui-input-block">
